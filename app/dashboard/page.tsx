...
import ProductsCharts from "../components/productsChart";
...
const Dashboard = async () => {
  ...
  const lowStockPercentage = allProducts.filter(
    (p) => Number(p.quantity) <= 5 && Number(p.quantity) >= 1
  ).length;
  const outOfStockPercentage = allProducts.filter(
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-gray-900">New products per week</h2>
            </div>
            <div className="h-48">
              <ProductsCharts data={weekleyProductData} />
            </div>
          </div>
        </div>
